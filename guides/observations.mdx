---
title: "Fetching Current Weather Observations"
...
func _on_observations(obs: ObservationPacket) -> void:
    ...
    if obs.windSpeed != 0.0:
        print("Wind: %.1f km/h from %d°" % [obs.windSpeed, obs.windDirection])
    if obs.windGust != 0.0:
        print("Gusts to: %.1f km/h" % obs.windGust)
...
| `windSpeed` | `float` | Wind speed in km/h. May be `0.0` if calm or unreported. |
| `windGust` | `float` | Wind gust speed in km/h. May be `0.0` if no gusts. |
